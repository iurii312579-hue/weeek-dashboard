...
Забирает задачи из Weeek API и генерирует HTML-дашборд в папку ./output/index.html
"""
...
def build_html(tasks, generated_at):
    ...
    for t in tasks:
        ...
        if c in col_total:
            col_total[c]  += 1
            if t.get("isCompleted"): col_closed[c] += 1
            else:                    col_open[c]   += 1
    ...
    # Просроченные задачи — сортировка по дате
    ...
    def overdue_rows():
        ...
        for t in overdue_sorted:
            ...
            num   = (t["title"].split(".")[0] if "." in t["title"][:6] else "—")
            title = t["title"][len(num)+1:].strip() if num != "—" else t["title"]
            ...
            <tr>
              ...
              <td style="padding:7px 8px;white-space:nowrap;color:#A32D2D;font-size:12px;">{t.get("dueDate","")}</td>
              <td style="padding:7px 8px;font-size:12px;color:#444;">{names}</td>
            </tr>""")
        return "\n".join(rows)

    def bar_rows(data, max_val, fill_color, text_color):
        rows = []
        for uid, cnt in data:
            w = round(cnt / max_val * 100)
            rows.append(f"""
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:7px;">
              <span style="width:155px;font-size:12px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="{user_name(uid)}">{user_name(uid)}</span>
              <div style="flex:1;background:#f0f0ee;border-radius:3px;height:18px;">
                <div style="width:{w}%;background:{fill_color};border-radius:3px;height:100%;display:flex;align-items:center;padding-left:6px;min-width:22px;">
                  <span style="font-size:11px;font-weight:600;color:{text_color};">{cnt}</span>
                </div>
              </div>
            </div>""")
        return "\n".join(rows)
...
<div class="container">
  ...
  <div class="header">
    <div>
      ...
      <div class="header-sub">Обновлено: {generated_at} &nbsp;·&nbsp; Всего задач: {total}</div>
    </div>
  ...
  <div class="metrics">
    <div class="metric">
      ...
      <div class="metric-value" style="color:#3B6D11;">{len(closed)}</div>
    ...
    <div class="metric">
      ...
      <div class="metric-value" style="color:#185FA5;">{len(opened)}</div>
    ...
    <div class="metric">
      ...
      <div class="metric-value" style="color:#A32D2D;">{len(overdue)}</div>
    ...
    <div class="metric">
      ...
      <div class="metric-value" style="color:#5F5E5A;">{len(no_due)}</div>
  ...
  <div class="grid2">
    <div class="card">
      ...
      {bar_rows(top_all, max_all, "#B5D4F4", "#0C447C")}
    </div>
    <div class="card">
      <div class="card-title"><span class="dot" style="background:#E24B4A;"></span>Просрочки по исполнителям</div>
      {bar_rows(top_overdue, max_ovd, "#F7C1C1", "#A32D2D") if top_overdue else '<p style="font-size:13px;color:#aaa;">Просроченных задач нет</p>'}
    </div>
  </div>

  <div class="card">
    <div class="card-title"><span class="dot" style="background:#E24B4A;"></span>Просроченные задачи ({len(overdue)})</div>
    {"<p style='font-size:13px;color:#aaa;'>Просроченных задач нет</p>" if not overdue else f"""
    <table>
      <thead><tr><th>№</th><th>Задача</th><th>Срок</th><th>Ответственный</th></tr></thead>
  ...
  <div class="grid2">
    <div class="card">
      <div class="card-title">Задач по колонкам</div>
...
<script>
  ...
  const tc = '#888780', gc = 'rgba(0,0,0,0.05)';
  ...
  new Chart(document.getElementById('statusChart'), {{
    ...
    data:{{ labels:colLabels, datasets:[
      {{ label:'Открыто', data:{col_open_d}, backgroundColor:'#85B7EB', borderRadius:3 }},
      {{ label:'Закрыто', data:{col_clos_d}, backgroundColor:'#97C459', borderRadius:3 }},
    ]}},
    options:{{ responsive:true, maintainAspectRatio:false,
...
def main():
    ...
    for col in COLS:
        ...
        print(f"  Колонка {COL_NAMES[col]}: {len(batch)} задач")
